...
describe "Hook Manager", "operations" do
  ...
  after(:each) do
    @hook_manager.uninstall_hook(
      @hook_manager.make_install_infos(
        @hook_manager.prepare_paths_to_install('./installer/spec/data/install_list')
      )
    )
  end
  ...
  it "설정 파일 내용으로부터 훅을 설치하는 데 필요한 정보를 계산할 수 있다" do
    @hook_manager.make_install_infos(
      @hook_manager.prepare_paths_to_install('./installer/spec/data/install_list2')
  ...
  it "훅을 설치할 경로와 발동 조건 정보를 파일에서 읽어올 수 있다" do
    @hook_manager.prepare_paths_to_install('./installer/spec/data/install_list').should == [['/tmp/test_install_git_hook', @trigger_point, @hook_name], ['/tmp/test_install_git_hook2', @trigger_point, @hook_name]]
  end

  it "훅이 설치되었는지 확인할 수 있다" do
    @hook_manager.check_hooks_installed('./installer/spec/data/install_list')
  end
end
